    ...
    def __init__(self, imgSize, backbone, inputChannel, outputChannel, rnnHiddenSize, numChars):
        ...
        if backbone == "vgg":
            ...
            self.featureExtractor = nn.Sequential(
                *list(models.resnet18(weights='IMAGENET1K_V1').children())[:-3],
            ...
            outputChannel = 256
            self.featureExtractor = nn.Sequential(
                *list(models.resnet34(weights='IMAGENET1K_V1').children())[:-3],
            ...
            outputChannel = 1024
            self.featureExtractor = nn.Sequential(
                *list(models.resnet50(weights='IMAGENET1K_V1').children())[:-3],
        ...
        if (backbone[:6] == "resnet") and (inputChannel == 1):
            self.featureExtractor[0] = nn.Conv2d(inputChannel, 64, kernel_size=(7, 7), stride=(2, 1), padding=(3, 1), bias=False)
...
if __name__ == '__main__':
    inputChannel = 1
    model = Model((64, 768), "resnet50", inputChannel, 256, 256, 199)
    output = model(torch.Tensor(1, inputChannel, 64, 768))
    print(output.shape)